...
class ProdukController extends Controller { 
    ...
    public function store(Request $request) { 
        $validatedData = $request->validate([ 
            ...
            if ($request->file('foto')) { 
                ...
                $validatedData['foto'] = $originalFileName; } Produk::create($validatedData, $messages); 
    ...
    public function edit(string $id) { 
        ...
        return view('backend.v_produk.edit', [ 
            'judul' => 'Ubah Produk', 
            'edit' => $produk, 'kategori' => $kategori ]); 
    }
    /** * Update the specified resource in storage. */ 
    public function update(Request $request, string $id) { 
        //ddd($request); 
            $produk = Produk::findOrFail($id);$rules = [ 
    ...
    public function destroy($id) { 
        ...
        if ($produk->foto) { 
            ...
            if (file_exists($thumbnailLg)) { unlink($thumbnailLg); 
            } 
            // Hapus thumbnail md 
            $thumbnailMd = $directory . 'thumb_md_' . $produk->foto; 
            if (file_exists($thumbnailMd)) { unlink($thumbnailMd); 
            } 
            // Hapus thumbnail sm 
            $thumbnailSm = $directory . 'thumb_sm_' . $produk->foto; 
            if (file_exists($thumbnailSm)) { unlink($thumbnailSm); 
            } 
        } 
    ...
    public function destroyFoto($id) { 
        ...
        $foto->delete(); return redirect()->route('backend.produk.show', $produkId) ->with('success', 'Foto berhasil dihapus.'); } }